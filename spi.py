...
class SPIRAM: # spiram is used as a hardware buffer  to play out the teletext packets.
  ...
  MODE_BYTE = 0x00
  MODE_PAGE = 0x80
  MODE_SEQUENTIAL = 0x41
  
  def __init__(self, bus, device):
    GPIO.setup(12, GPIO.OUT) # GPIO_CSN. Chip select is bit banged because reasons
    ...
    #self.spi.max_speed_hz=122000 # slowest that will work

  # chip select
  def toggleCS(self): # /// Toggle the chip select prior to setting state
    # deselect and reselect the chip prior to setting it up
    GPIO.output(12, GPIO.HIGH) # CSN
    time.sleep(0.000110)
    GPIO.output(12, GPIO.LOW)
  def deselect(self):
    GPIO.output(12, GPIO.HIGH) # disable the spiram CSN
  # addressing
